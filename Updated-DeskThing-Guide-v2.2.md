...
**Updated v2.2:** Added iframe focus bug documentation and fix based on ThetisThing v2.0.1 button fix; corrected button identifiers (Digit1-4 not m1-m4); documented two-path button event handling
...
### Rule 9: Handle Both Button Event Paths
Physical buttons fire through TWO different event types depending on app focus state:
```javascript
// Handler for when app is FOCUSED
DeskThing.on("button", (data) => {
  const button = data?.payload?.button; // "Digit1", "Digit2", etc.
  // Handle button press
});

// Handler for mapped actions (when NOT focused)
DeskThing.on(DESKTHING_EVENTS.ACTION, (data) => {
  const actionId = data?.payload?.id;
  // Handle action
});
```
**Note:** Button identifiers are `Digit1` through `Digit4`, NOT `m1` through `m4`.

### Rule 10: Add Focus Management for Non-React Clients
If using inline `onclick` handlers, add focus management code to prevent physical buttons from stopping after screen touch. See Section 17 for full details.

### Rule 11: NEVER Use modes: ["default"] in Key Registration
```javascript
...
### ⚠️ IMPORTANT: Focus Management for Physical Buttons

If your app uses inline `onclick` handlers or non-React patterns, physical buttons may stop working after touching the screen. This is due to iframe focus issues. See **Section 17: Bug: Physical Buttons Stop Working After Touching Screen** for the full explanation and fix.

**Quick Check:** If physical buttons work until you touch the screen, then stop working, you need focus management code in your client.
...
**IMPORTANT:** Button identifiers are `Digit1` through `Digit4`, NOT `m1` through `m4` as some older documentation suggests.

```javascript
DeskThing.on("button", (data) => {
  const button = data?.payload?.button || data?.button || data?.payload?.id || data?.id;
  const state = data?.payload?.state || data?.state; // "down" or "up"
  
  // Only act on button press (down), not release (up)
  if (state === "up") return;
  
  switch (button) {
    case "Digit1": case "1": case "preset1":
      handleButton1();
      break;
    case "Digit2": case "2": case "preset2":
      handleButton2();
      break;
    case "Digit3": case "3": case "preset3":
      handleButton3();
      break;
    case "Digit4": case "4": case "preset4":
      handleButton4();
      break;
    // Scroll wheel can also fire as button events
    case "ScrollUp": case "scroll_up":
      handleScrollUp();
      break;
    case "ScrollDown": case "scroll_down":
      handleScrollDown();
      break;
  }
});
...
### Two Event Paths for Button Handling

Physical buttons can reach your app through TWO different event types:

| Event | When It Fires | Handler |
|-------|---------------|---------|
| `"button"` | When your app is **focused** | `DeskThing.on("button", ...)` |
| `DESKTHING_EVENTS.ACTION` | When your app is **NOT focused** (via mappings) | `DeskThing.on(DESKTHING_EVENTS.ACTION, ...)` |

**You need BOTH handlers if you want buttons to work in all scenarios:**

```javascript
// Handler for when app is FOCUSED - receives raw button events
DeskThing.on("button", (data) => {
  const button = data?.payload?.button || data?.button;
  const buttonState = data?.payload?.state || data?.state;
  
  console.log(`[MyApp] BUTTON event: ${button} state=${buttonState}`);
  
  if (buttonState === "up") return;
  
  switch (button) {
    case "Digit1": handleButton1(); break;
    case "Digit2": handleButton2(); break;
    case "Digit3": handleButton3(); break;
    case "Digit4": handleButton4(); break;
  }
});

// Handler for mapped actions - fires when NOT focused or via explicit mappings
DeskThing.on(DESKTHING_EVENTS.ACTION, (data) => {
  const actionId = data?.payload?.id || data?.id;
  
  console.log(`[MyApp] ACTION event: ${actionId}`);
  
  switch (actionId) {
    case "button1Action": handleButton1(); break;
    case "button2Action": handleButton2(); break;
    // ... handle your registered actions
  }
});
```
...
### Bug: Physical Buttons Stop Working After Touching Screen

**Symptom:** Physical buttons (Digit1-4, scroll wheel) work perfectly UNTIL you touch any on-screen button. After touching the screen, physical buttons stop working. To restore them, you must switch apps or open the drawer.

**Cause:** Iframe focus loss. The DeskThing client loads apps in an iframe. When you touch an on-screen button:
1. Focus shifts to the iframe content
2. Physical button events (which come from the parent DeskThing client) no longer route to your app
3. The iframe is "holding" focus, blocking parent events

**Why Some Apps Are Affected and Others Aren't:**
- Apps using **inline onclick handlers** (`<button onclick="send('action')">`) are vulnerable
- Apps using **compiled React bundles** with proper DeskThing SDK integration typically avoid this issue
- The difference is in how focus is managed after touch interactions

**Solution: Add Focus Management to Client**

Add this code to your client's JavaScript (at the end of your script section):

```javascript
// FOCUS MANAGEMENT FIX: Restore focus after touch/click events
// This fixes the issue where physical buttons stop working after touching the screen
(function initFocusManagement() {
  var refocusTimeout = null;
  
  function restoreFocus() {
    // Clear any pending refocus to avoid multiple calls
    if (refocusTimeout) {
      clearTimeout(refocusTimeout);
    }
    // Slight delay to allow click handler to complete
    refocusTimeout = setTimeout(function() {
      try {
        // Focus the iframe window first
        window.focus();
        // Then try to blur any focused element to release focus
        if (document.activeElement && document.activeElement !== document.body) {
          document.activeElement.blur();
        }
        // Notify parent to refocus if needed
  ...
  // Listen for touch and click events on the entire document
  document.addEventListener('touchend', restoreFocus, { capture: true, passive: true });
  document.addEventListener('click', restoreFocus, { capture: true, passive: true });
  document.addEventListener('mouseup', restoreFocus, { capture: true, passive: true });
  
  // Also handle focus events to prevent iframe from holding focus
  document.addEventListener('focusin', function(e) {
    // Don't prevent focus on input fields if you have any
    if (e.target.tagName !== 'INPUT' && e.target.tagName !== 'TEXTAREA') {
      restoreFocus();
    }
  }, { capture: true, passive: true });
  
  console.log('[MyApp] Focus management initialized');
})();
```

**Alternative Solutions (if focus management doesn't work):**

1. **Remove inline onclick handlers** - Use addEventListener instead:
```javascript
// ❌ WRONG - Creates focus issues
<button onclick="send('action')">Click</button>

// ✅ BETTER - Doesn't capture focus the same way
document.getElementById('myButton').addEventListener('click', () => {
  send('action');
});
```

2. **Rebuild as proper React app** - Use the `deskthing-client` npm module like FlexThing does:
```javascript
// React approach - SDK handles focus properly
import DeskThing from 'deskthing-client';

function MyButton() {
  return (
    <button onClick={() => DeskThing.send({ type: 'action' })}>
      Click
    </button>
  );
}
...
Add this to see what's happening with focus:
```javascript
document.addEventListener('focusin', (e) => {
  console.log('[Focus] Focus moved to:', e.target.tagName, e.target.id || e.target.className);
});

document.addEventListener('focusout', (e) => {
  console.log('[Focus] Focus left:', e.target.tagName, e.target.id || e.target.className);
});
```
```
